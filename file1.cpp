...
void showTable(const string &command);
...
int main()
{
    ...
    for (auto &cmd : commands)
    {
        ...
        else if (upper.find("TABLES") != string::npos)
        {
            showTable(cmd);
            need_pass = true;
        }
...
void updateTable(const string &command)
{
    ...
    // cout << endl << endl << "222222222222222222222";
...
void countRows(const string &command)
{
    ...
    ostringstream oss;
    oss << g_tables[idx].table.data.size() << endl
        << endl;
    appendToFile(g_outputFile, oss.str());
}
